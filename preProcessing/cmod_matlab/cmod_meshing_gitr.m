...
for i=1:length(planes)
    ...
    samples = sample_triangle(x_tri,y_tri,z_tri,4)
    xP((i-1)*4 +1:i*4) = samples(:,1);
    yP((i-1)*4 +1:i*4) = samples(:,2);
    zP((i-1)*4 +1:i*4) = samples(:,3);
    vxP((i-1)*4 +1:i*4) = 50*normal(1); %[m/s]
    vyP((i-1)*4 +1:i*4) = 50*normal(2); %[m/s]
    vzP((i-1)*4 +1:i*4) = 50*normal(3); %[m/s]
end
